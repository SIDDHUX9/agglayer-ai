...
export function LiveActivityFeed() {
  ...
  const [activities, setActivities] = useState<ActivityItem[]>([
    ...
    { id: "2", type: "deposit", message: "Demo deposit on testnet", time: "5 mins ago", amount: "5.2 mUSDC" },
    { id: "3", type: "rebalance", message: "Optimized Aave position", time: "12 mins ago", amount: "$12,500" },
    { id: "4", type: "deposit", message: "Demo deposit on testnet", time: "15 mins ago", amount: "1.5 mUSDC" },
    { id: "5", type: "alert", message: "Testnet simulation active", time: "22 mins ago" },
  ]);
  ...
  // Simulate live updates with realistic demo data
  useEffect(() => {
    const interval = setInterval(() => {
      const newActivity: ActivityItem = Math.random() > 0.6
        ? {
            ...
            type: "deposit",
            message: "Demo testnet deposit",
            time: "Just now",
            amount: `${(Math.random() * 10).toFixed(2)} mUSDC`
          }
        : {
            id: Date.now().toString(),
            type: "rebalance",
            message: "Demo rebalance simulation",
            time: "Just now",
            amount: `$${(Math.random() * 5000).toFixed(0)}`
          };

      setActivities(prev => [newActivity, ...prev.slice(0, 9)]);
    }, 15000); // Every 15 seconds for more realistic pacing
