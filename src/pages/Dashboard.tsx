...
import { CONTRACTS, MASTER_VAULT_ABI, MATIC_VAULT_ABI } from "@/lib/contracts";
...
export default function Dashboard() {
  ...
  // Read real TVL from MATIC vault
  const { data: maticVaultTvl } = useReadContract({
    ...
    query: {
      enabled: CONTRACTS.MATIC_VAULT.address.length > 0,
    },
  });

  // Read user's MATIC vault shares
  const { data: userMaticShares } = useReadContract({
    ...
    chainId: CONTRACTS.MATIC_VAULT.chainId,
    query: {
      enabled: CONTRACTS.MATIC_VAULT.address.length > 0 && !!address,
    },
  });
  ...
  const maticPriceUsd = 0.80; // Approximate MATIC price for display (could fetch from API)
  const maticTvlInUsd = maticTvlInMatic * maticPriceUsd;
  const totalTvl = usdcTvlInUsd + maticTvlInUsd;

  // Real APY from MockAaveAdapter (5% hardcoded in contract)
  const avgApy = 5.0;
  ...
  // Gas savings: Rebalancing on single chain vs multi-chain
  // Estimate: ~$2.50 per rebalance saved by using AggLayer architecture
  const estimatedRebalances = Math.floor(totalTvl / 1000); // Assume 1 rebalance per $1000 TVL
  const gasSaved = estimatedRebalances * 2.5;

  // Calculate TVL growth based on APY
  const tvlGrowth = avgApy.toFixed(1);

  // APY comparison with market (MockAaveAdapter provides 5% vs typical 3-4%)
  const marketAvgApy = 3.5; // Typical single-chain DeFi APY
  const apyDiff = (avgApy - marketAvgApy).toFixed(1);

  // REMOVED LOADING SCREEN - Show dashboard immediately with loading states for individual components
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 w-full py-8 px-4 lg:px-8 relative z-10">
        ...
        {/* My Vaults Section - Prominent Access */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Access your USDC and MATIC vaults</p>
            </div>
          </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <a
                      href={CONTRACTS.MASTER_VAULT.explorer}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="font-mono text-xs text-primary hover:underline"
                    >
                      {CONTRACTS.MASTER_VAULT.address.slice(0, 6)}...{CONTRACTS.MASTER_VAULT.address.slice(-4)}
                    </a>
                  </div>
                </div>
                {vaults?.[0] && (
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Deposit MATIC directly</CardDescription>
                  </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">TVL</span>
                    <span className="font-semibold">
                      {maticVaultTvl ? `${formatEther(maticVaultTvl as bigint)} MATIC` : "Loading..."}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Your Balance</span>
                    <span className="font-semibold">
                      {userMaticShares ? `${formatEther(userMaticShares as bigint)} agMATIC` : "0"}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Chain</span>
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Contract</span>
                    <a
                      href={CONTRACTS.MATIC_VAULT.explorer}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="font-mono text-xs text-primary hover:underline"
                    >
                      {CONTRACTS.MATIC_VAULT.address.slice(0, 6)}...{CONTRACTS.MATIC_VAULT.address.slice(-4)}
                    </a>
                  </div>
                </div>
                {vaults?.[0] && (
                  <DepositDialog vaultId={vaults[0]._id} vaultName="MATIC Vault" />
                )}
              </CardContent>
            </Card>
          </div>
        ...
        >
          <div className="flex items-center justify-center gap-4 flex-wrap text-xs text-muted-foreground">
            ...
            <span>•</span>
            <a
              href="https://amoy.polygonscan.com/address/0x831F6F30cc0Aa68a9541B79c2289BF748DEC4a2a"
              target="_blank"
              rel="noopener noreferrer"
              className="text-primary hover:underline"
            >
              USDC Vault: 0x831F...2a2a
            </a>
            <span>•</span>
            <a
              href="https://amoy.polygonscan.com/address/0xd399F27f84A5928460b8f9f222EBcb4438F716b5"
              target="_blank"
              rel="noopener noreferrer"
              className="text-primary hover:underline"
            >
              MATIC Vault: 0xd399...16b5
            </a>
            <span>•</span>
